...
const state = {
  ...
  startEndeUhrzeitIntervalls: DefaultObjectCreator.createDefaultStartEndeUhrzeitIntervalls()
}
...
const getters = {
  ...
  getZaehlungById(state: any) {
    return (id: string): LadeZaehlungDTO | undefined => {
      const selected = state.zaehlungen.filter((z: LadeZaehlungDTO) => z.id === id) as LadeZaehlungDTO[];
      if (selected.length === 1) {
        return selected[0];
      } else if (selected.length > 1) {
        console.warn(`Es wurden mehr als eine Zählung mit der ID '${id} gefunden!'`)
        return selected[0];
      }
...
const mutations = {
  setZaehlungen(state: any, payload: { zs: LadeZaehlungDTO[], isAnwender: boolean }) {
    if (payload.isAnwender) {
      // Alles Sonderzaehlungen ausfiltern
      state.zaehlungen = payload.zs.filter(value => !value.sonderzaehlung);
    } else {
  ...
  setZaehlungAlsAktiv(state: any, id: string) {
    if (state.zaehlungen.length > 0) {
      ...
      if (id) {
        ...
        aktiv = state.zaehlungen.filter((z: LadeZaehlungDTO) => z.id === id) as LadeZaehlungDTO[];
      } else {
        // ansonsten ist es die neuste Zählung
        aktiv.push(state.zaehlungen.sort(LadeZaehlungComperator.sortByDatum)[0]);
      }
      ...
      if (aktiv.length > 0) {
        ...
        state.sortedKnotenarme = aktiv[0].knotenarme.sort(LadeKnotenarmComperator.sortByNumber);
        // inaktive Zählungen aktualisieren und sortieren
        const iz = state.zaehlungen.filter((z: LadeZaehlungDTO) => z.id != state.aktiveZaehlung.id) as LadeZaehlungDTO[];
        state.inaktiveZaehlungen = iz.sort(LadeZaehlungComperator.sortByDatum);
        ...
        console.warn("Es konnte keine Zählung für die ID '" + id + "' gefunden werden.");
      }
    } else {
      console.warn("Es sind keine Zählungen zu dieser Zählstelle vorhanden.");
    }
  },
  setStartEndeUhrzeitIntervalls(state: any, payload: StartEndeUhrzeitIntervalls) {
    state.startEndeUhrzeitIntervalls = payload;
  }
}

const actions = {
  setZaehlungen(context: any, payload: { zs: LadeZaehlungDTO[], id: string, isAnwender: boolean }) {
    context.commit("setZaehlungen", {zs: payload.zs, isAnwender: payload.isAnwender});

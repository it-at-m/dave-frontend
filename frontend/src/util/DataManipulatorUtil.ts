...
export default class DataManipulatorUtil {
  ...
  public static manipulateEndeUhrzeit(zaehldatum: LadeZaehldatumDTO): string {
    ...
    if (zaehldatum.endeUhrzeit === DataManipulatorUtil.TIME_VALUE_FOUND_END_OF_DAY
        || (zaehldatum.endeUhrzeit < zaehldatum.startUhrzeit
            && zaehldatum.endeUhrzeit === DataManipulatorUtil.TIME_VALUE_FOUND_START_OF_DAY)
    ) {
      manipualtedEndeUrhzeit = DataManipulatorUtil.TIME_VALUE_TARGET_END_OF_DAY;
    } else {

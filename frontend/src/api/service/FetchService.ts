...
export default class FetchService {
  ...
  private static readonly RESPONSE_TYPE_OPAQUE_REDIRECT: string = "opaqueredirect";
        ...
        .then(response => {
          if (!response.ok) {
            if (response.status === 400) {
              throw new ApiError(Levels.ERROR, errorMessage, `Fehlerhafte Anfrage an das Backend geschickt, HTTP: 400.`)
            ...
            } else if (response.type === FetchService.RESPONSE_TYPE_OPAQUE_REDIRECT) {
              location.reload();
            }
            throw new ApiError(Levels.ERROR, errorMessage, `Fehler: ${response.status} ${response.statusText}`);
          }
  ...
  static postForPdf(dataToSave: any, endpoint: string, errorMessage: string): Promise<any> {
    const url = `${this.BASE}/${endpoint}`;
    const request = FetchUtils.getPdfPOSTConfig(dataToSave);
    return FetchService.sendRequestForPDF(url, request, errorMessage);
  }

  private static sendRequestForPDF(url: string, request: RequestInit, errorMessage: string): Promise<any> {
    return fetch(url, request)
        .catch(error => {
          throw new ApiError(Levels.ERROR, `Die Verbindung zum Service konnte nicht aufgebaut werden.`, error);
        })

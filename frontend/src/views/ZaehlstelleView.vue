...
export default class ZaehlstelleView extends Vue {
  ...
  private hasZaehlungen: boolean = true;
  ...
  created() {
    ...
    ZaehlstellenService.getById(zaehlstelleId).then(zaehlstelle => {
      ...
      if (this.$store.getters["security/isNoSecurityActive"]) {
        this.$store.dispatch("setZaehlungen", {zs: zaehlungen, id: this.$route.params.zaehlungId, isAnwender: false});
      } else {
  ...
  get kreuzungsname(): string {
    ...
    let aktiveZaehlung: LadeZaehlungDTO = this.$store.getters.getAktiveZaehlung;
    if (aktiveZaehlung) {
  ...
  get zInfoHeight(): number {
    ...
    return (72 + 144 + anzahlKnotenarme * 24) / (this.$vuetify.breakpoint.height / 100)
  }
  ...
  get timelineHeight(): number {
    return 100 - this.appBarHeight - this.headerHeight - this.zInfoHeight - (56 / (this.$vuetify.breakpoint.height / 100))
  }
  ...
  get rightContentHeightVh(): string {
    const h = 100 - this.headerHeight - this.appBarHeight - (72 / (this.$vuetify.breakpoint.height / 100))
    return h + "vh"
  }

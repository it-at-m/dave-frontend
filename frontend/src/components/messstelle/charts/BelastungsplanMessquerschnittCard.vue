      ...
      SVG.SVG()
        .polygon(
          `${startX.value + 20},${startY.value + 451} ${
            startX.value
          },${startY.value + 451} ${startX.value + 10} ${startY.value + 460}`
        )
    ...
    querschnittGroup.value.add(
      SVG.SVG()
        .line(
          ...
          startY.value + 460,
          startX.value - 25,
          startY.value + (650 - (3 - numberOfChosenFahrzeugOptions.value) * 65)
        )
        .stroke({ width: 1, color: "black" })
    );
    addTextSouthSide(
  ...
  canvas.value
    .text(function (add) {
      ...
      if (chosenFahrzeugartAsTextArray.length != 0) {
        ...
        add.tspan("").newLine();
        add
          .tspan(chosenFahrzeugartAsTextArray[0])
          .newLine()
          .x(textstart)
          .font({ weight: "bold" });
        if (chosenFahrzeugartAsTextArray.length >= 2) {
          textstart += 60;
          add
            .tspan(`(${chosenFahrzeugartAsTextArray[1]})`)
            .x(textstart)
            .font({ weight: "bold" });
        }
        if (chosenFahrzeugartAsTextArray.length == 3) {
          textstart += 60;
          add
            .tspan(chosenFahrzeugartAsTextArray[2])
            .x(textstart)
            .font({ weight: "bold" });
        }
...
const getZeitblockText = computed(() => {
  if (chosenOptionsCopy.value.zeitauswahl == Zeitauswahl.TAGESWERT) {
    ...
    return zeitblockStuendlichInfo.get(chosenOptionsCopy.value.zeitblock)?.text;
  } else {
    return "";
  }
});

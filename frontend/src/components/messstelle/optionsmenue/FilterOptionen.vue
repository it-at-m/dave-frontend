<template>
  ...
  >
    ...
    >
      ...
      <v-col cols="2">
        <span :class="getStyleClass(messquerschnitt.mqId) + ' hidden-xl-only'"
          >[
          {{ himmelsRichtungenTextShort.get(messquerschnitt.fahrtrichtung) }}
          ]</span
        >
        <span
          :class="getStyleClass(messquerschnitt.mqId) + ' hidden-lg-and-down'"
          >[
          {{ himmelsRichtungenTextLong.get(messquerschnitt.fahrtrichtung) }}
          ]</span
        >
      </v-col>
    </v-row>
...
function getStyleClass(mqId: string): string {
  const notIncluded = "text-caption grey--text text--lighten-1";
  const included = "text-caption font-weight-medium white--text";

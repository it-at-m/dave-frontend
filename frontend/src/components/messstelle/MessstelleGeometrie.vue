...
const includesKnotenarm1and3 = computed(() => {
  ...
  props.knotenarme.forEach((q) => {
    north = north || q.fahrtrichtung === Himmelsrichtungen.NORD;
    south = south || q.fahrtrichtung === Himmelsrichtungen.SUED;
  });
  return north && south;
});
...
const includesKnotenarm2and4 = computed(() => {
  ...
  let west = false;
  props.knotenarme.forEach((q) => {
    east = east || q.fahrtrichtung === Himmelsrichtungen.OST;
    west = west || q.fahrtrichtung === Himmelsrichtungen.WEST;
  });
  return east && west;
});
</script>

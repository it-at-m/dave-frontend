...
export default class History extends Vue {
  ...
  storeZaehlung(zaehlung: LadeZaehlungDTO): void {
    ...
    if (zaehlung && zaehlstelle) {
      ...
      console.warn("Die Zählung, bzw. die Zählstelle konnte nicht aus dem Store geladen werden.")
    }
  ...
  addOptionen(optionen: OptionsDTO): void {
    if (optionen) {
      ...
      console.warn("Die Optionseinstellungen für den History Eintrag konnten nicht gespeichert werden.")
    }
  ...
  convertDate(date: string): Date {
    if (!Date.parse(date)) {
      console.warn(`Datum der Zählung (${date}) konnte nicht gelesen werden.`)
      return new Date()
    }

<template>
  ...
  >
    ...
    >
      ...
      <l-layer-group
          v-for="(zaehlartenKarteDto, indexZaehlarten) in getSelectedZaehlstelleKarte.zaehlartenKarte"
          :key="indexZaehlarten"
      >
        <l-marker
            v-for="(zaehlart, indexZaehlart) in zaehlartenKarteDto.zaehlarten"
            :key="indexZaehlart"
            :lat-lng="createLatLngAsArray(zaehlartenKarteDto)"
            @click="choosenZaehlartIconToZaehlstelleHeader(zaehlart)"
        >
          <l-icon
              :icon-anchor="calculateIconAnchorCoordinatesForZaehlartMarker(indexZaehlart)"
          >
            <v-avatar
...
export default class ZaehlstelleMap extends Vue {
  ...
  private static readonly ICON_ANCHOR_OFFSET_PIXELS_ZAEHLART_MARKER: number = -32;
  ...
  private selectedZaehlstelleKarte: ZaehlstelleKarteDTO = DefaultObjectCreator.createDefaultZaehlstelleKarte();
  ...
  mounted() {
    SucheService.searchZaehlstelle(this.$store.getters["search/lastSearchQuery"])
        .then(result => {
  ...
  private setMarkerToMap() {
    ...
    this.selectedZaehlstelleKarte = DefaultObjectCreator.createDefaultZaehlstelleKarte();
    ...
    const zaehlstellenKarte: ZaehlstelleKarteDTO[] = this.getZaehlstellenKarteFromStore;
    ...
    if (zaehlstellenKarte.length === 1) {
       ...
       */
      this.theMap.mapObject.setView(this.createLatLng(zaehlstellenKarte[0]), 18);
    } else if (!this.zId) {
  ...
  private createLatLng(zaehlstelleKarte: ZaehlstelleKarteDTO): LatLng {
    return this.createLatLngFromString(zaehlstelleKarte.latitude, zaehlstelleKarte.longitude);
  }
  ...
  private createLatLngAsArray(zaehlartenKarte: ZaehlartenKarteDTO): Array<number> {
    return [
  ...
  private createMarkerForZaehlstelle(zaehlstelleKarte: ZaehlstelleKarteDTO): Marker {
   ...
   */
  private calculateIconAnchorCoordinatesForZaehlartMarker(index: number): Array<number> {
    let xCoordinate: number = ZaehlstelleMap.ICON_ANCHOR_INITIAL_OFFSET_PIXELS_ZAEHLART_MARKER
        + (index % ZaehlstelleMap.NUMBER_OF_COLUMNS_ZAEHLART_MARKER)
        * ZaehlstelleMap.ICON_ANCHOR_OFFSET_PIXELS_ZAEHLART_MARKER;
    let yCoordinate: number = ZaehlstelleMap.ICON_ANCHOR_INITIAL_OFFSET_PIXELS_ZAEHLART_MARKER
        + Math.floor(index / ZaehlstelleMap.NUMBER_OF_COLUMNS_ZAEHLART_MARKER)
        * ZaehlstelleMap.ICON_ANCHOR_OFFSET_PIXELS_ZAEHLART_MARKER;
  ...
  private mapReady() {
    this.theMap.mapObject.addControl(control.attribution({
      position: 'bottomleft',
      prefix: ''
    }));

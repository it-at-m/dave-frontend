<template>
  <v-container fluid class="pa-0">
    <v-row no-gutters style="background-color: #EF9A9A"> <!-- red lighten-3 -->
      <v-col cols="12" lg="1" v-if="zaehlung.sonderzaehlung" align-self="center" :class="isLarge ? '' : 'px-1'">
        <sonderzaehlung-icon :large="isLarge"/>
      </v-col>

      <v-col cols="12" :lg="getMaxCols">
        ...
        >
          <v-row class="pa-0 ma-0" no-gutters>
            <v-col cols="12" lg="8">
              <v-row no-gutters><span
                  class="pb-0 text-truncate">{{ $d(datum, 'short', 'de-DE') }} | {{ zaehlung.projektName }}</span>
              </v-row>
              <v-row no-gutters class="ma-0">
                <zaehlart-icon :zaehlart="zaehlung.zaehlart" color="primary"></zaehlart-icon>
                <wetter-icon :wetter="zaehlung.wetter" color="primary"></wetter-icon>
                <zaehldauer-icon :zaehldauer="zaehlung.zaehldauer" color="primary"></zaehldauer-icon>
                <quelle-icon :quelle="zaehlung.quelle" color="primary"></quelle-icon>
                <kommentar-info
            ...
            <v-col lg class="hidden-md-and-down">
              <v-spacer></v-spacer>
            </v-col>
            <v-col lg class="hidden-md-and-down">
              <zaehlung-geometrie
    ...
    <v-row no-gutters>
      ...
      >
        <v-row>
          <v-col cols="1"></v-col>
          <v-col>
            <h3 class="grey--text text--lighten-1">Aktuelle Filtereinstellungen</h3>
          </v-col>
        </v-row>
        <v-row no-gutters>
          <v-col cols="1" align-self="start">
            <v-icon small color="grey lighten-1">mdi-clock-time-four-outline</v-icon>
          </v-col>
          <v-col cols="10">
            <span class="grey--text text--lighten-1">Zeitauswahl: <span
                class="font-weight-medium white--text">{{ zeitblock }}</span> in <span
                class="font-weight-medium white--text">{{ zeitintervall }}</span> Intervallen</span>
          </v-col>
        </v-row>
        <v-row no-gutters v-if="isSpitzenstundeInZeitauswahlChosen && showSpitzenstundeInfo">
          <v-col cols="1" align-self="start">
            <v-icon small color="grey lighten-1">mdi-summit</v-icon>
          </v-col>
          <v-col cols="10">
            <span class="grey--text text--lighten-1">{{ options.zeitauswahl }}:
        ...
        <v-row no-gutters class="ma-0" v-for="(k, index) in sortedKnotenarme" v-bind:key="k.nummer">
          <v-col cols="1">
            <v-icon v-if="index === 0" small color="grey lighten-1">mdi-arrow-decision</v-icon>
          </v-col>
          <v-col cols="9">
            <!-- Wird angezeigt in Viewport Größe lg und darüber -->
            <span v-if="options.vonIds.includes(k.nummer)"
                  class="caption font-weight-medium white--text hidden-md-and-down">von </span>
            <span v-if="options.nachIds.includes(k.nummer)"
                  class="caption font-weight-medium white--text hidden-md-and-down">nach </span>
            <!-- Wird angezeigt in Viewport Größe md und drunter -->
            <span v-if="options.vonIds.includes(k.nummer)"
                  class="caption font-weight-medium white--text hidden-lg-and-up">v </span>
            <span v-if="options.nachIds.includes(k.nummer)"
                  class="caption font-weight-medium white--text hidden-lg-and-up">n </span>
            <span class="caption grey--text text--lighten-1">{{ k.nummer }} {{ k.strassenname }}</span>
          </v-col>
          <v-col cols="2">
            <span
                class="caption grey--text text--lighten-1 hidden-xl-only">[ {{
                himmelsRichtungen.get(k.nummer).s
              }} ]</span>
            <span
                class="caption grey--text text--lighten-1 hidden-lg-and-down">[ {{ himmelsRichtungen.get(k.nummer).l }} ]</span>
          </v-col>
        </v-row>
...
export default class ZaehlungInfo extends Vue {
  ...
  private showSpitzenstundeInfo: boolean = false;

  private startUhrzeitIntervalls: string = "";

  private endeUhrzeitIntervalls: string = "";
  ...
  showStartEndeUhrzeit(startEndeUhrzeitIntervalle: StartEndeUhrzeitIntervalls) {
    this.startUhrzeitIntervalls = startEndeUhrzeitIntervalle.startUhrzeitIntervalls;
    this.endeUhrzeitIntervalls = startEndeUhrzeitIntervalle.endeUhrzeitIntervalls;
    this.showSpitzenstundeInfo = !(_.isEmpty(this.startUhrzeitIntervalls) || _.isEmpty(this.endeUhrzeitIntervalls));
  }
  ...
  get zeitblock(): string | undefined {
    ...
    if (this.zaehlung.zaehldauer !== Zaehldauer.DAUER_24_STUNDEN && this.options.zeitblock === Zeitblock.ZB_00_24) {
      return 'Tageswert';
    }
    ...
    console.warn("Es wurde in den Optionen kein passender Zeitblock gefunden.")
  ...
  get isSpitzenstundeInZeitauswahlChosen(): boolean {
    return this.options.zeitauswahl === Zeitauswahl.SPITZENSTUNDE_KFZ

<template>
  ...
  >
    ...
    >Für diese Zählstelle sind keine weiteren Zählungen vorhanden.</span>
    </v-sheet>
    <v-tooltip
        right
        class="pl-5"
...
export default class ZaehlungenTimeline extends Vue {
  ...
  updateQuery(n: string) {
    ...
    if (q) {
      ...
      if (this.filteredZaehlungen.length > 0) {
        ...
        this.$store.dispatch("setZaehlungAlsAktiv", this.filteredZaehlungen[0].id)
      }
    } else {
  ...
  get filteredZaehlungen(): LadeZaehlungDTO[] {
    let filteredZs = this.$store.getters.getInaktiveZaehlungen as LadeZaehlungDTO[]
    if (this.query.length > 0) {
      if (!_.isNil(this.externalQuery) && (this.query === this.externalQuery || this.query == zaehlartText.get(this.externalQuery))) {
        filteredZs = filteredZs.filter(z =>
            z.zaehlart.toLowerCase() === this.externalQuery!.trim().toLowerCase() &&
            // Suche nach einem Zählartbuchstaben ist manchaml schwierig,
            // deshalb kann hier nach dem ganzen Text gesucht werden
            zaehlartText.get(z.zaehlart)?.toLowerCase().includes(this.query.trim().toLocaleLowerCase())
        );
      } else {
        const words: String[] = this.query.split(" ");
        words.forEach(w => {
          filteredZs = filteredZs.filter(z =>
              new Date(z.datum).toLocaleDateString('de-De', this.dateOptions).includes(w.toLowerCase()) ||
              ...
              // Suche nach einem Zählartbuchstaben ist manchaml schwierig,
              // deshalb kann hier nach dem ganzen Text gesucht werden
              zaehlartText.get(z.zaehlart)?.toLowerCase().includes(w.toLocaleLowerCase())
          );
        });
      }
    }

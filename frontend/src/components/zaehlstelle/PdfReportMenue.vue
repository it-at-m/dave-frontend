    ...
    >
      <v-card width="900px" flat>
        ...
        <v-list
            ...
            three-line
        >
          <v-subheader>Wählen Sie die Inhalte, die dem PDF Report hinzugefügt werden sollen.</v-subheader>
          <v-list-item>
            <v-list-item-action>
            ...
            <v-list-item-content>
              <v-list-item-title>Zählstelleninformationen</v-list-item-title>
              <v-list-item-subtitle>Folgende Informationen werden im PDF Report eingetragen: Zählstellen-Nummer,
                Knoten-/Platzname, Stadtbezirk, Zählstellenkommentar.
          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title>Zählungsinformationen</v-list-item-title>
              <v-list-item-subtitle>Folgende Informationen werden im PDF Report eingetragen: Projektname, Zählart,
                Zähldatum, Zähldauer, Zählsituation und erw. Zählsituation, Wetter, Quelle, Zählungskommentar,
                Knotenarmnummern und Straßenamen (1 bis 8), Hinweis auf eine etwaige Sonderzählung
              </v-list-item-subtitle>
            </v-list-item-content>
          </v-list-item>
          ...
          <v-list-item>
            <v-list-item-action>
              <v-checkbox v-model="zaehlungskenngroessen"></v-checkbox>
            </v-list-item-action>

            <v-list-item-content>
              <v-list-item-title>Zählungskenngrößen</v-list-item-title>
              <v-list-item-subtitle>Folgende Informationen werden im PDF Report eingetragen:
                Uhrzeiten Spitzenstunde (morgens, abends, Tag), Zählblöcke und Tageswert (nur wenn 24h Zählung), jeweils
                mit den jeweiligen Zählwerten für KFZ, SV und GV sowie Rad.
              </v-list-item-subtitle>
            </v-list-item-content>
          </v-list-item>
          ...
          <v-list-item>
            ...
            <v-list-item-content>
              ...
              <v-list-item-subtitle>Die Legende enthält Kurzbeschreibungen der einzelnen Zählattribute, z.B. für den
...
export default class PdfReportMenue extends Vue {
  ...
  zaehlstelleinfo: boolean = false
  zaehlungsinfo: boolean = false
  legende: boolean = false
  zaehlungskenngroessen: boolean = false;
  ...
  createZaehlstelleInfo() {
    ...
    const headline = new HeadingAsset(`Info für Zählstelle Nr. ${zs.nummer}`, AssetTypesEnum.HEADING3)
    const platz = new TextAsset(`Knoten-/Platzname: ${zl.kreuzungsname}`)
    const stadtbezirk = new TextAsset(`Stadtbezirk: ${zs.stadtbezirk} (${zs.stadtbezirkNummer})`)
    let kommentar = new TextAsset(`Zählstellenkommentar: ${this.checkForNull(zs.kommentar)}`)
    this.$store.dispatch("addAssets", [headline, platz, stadtbezirk, kommentar])
  }
  ...
  createZaehlungsInfo() {
    ...
    const headline = new HeadingAsset(`Info zur Zählung vom ${datum} (Zs-Nr. ${zs.nummer})`, AssetTypesEnum.HEADING3)
    const projektname = new TextAsset(`Projektname: ${zl.projektName}`)
    const zaehlart = new TextAsset(`Zählart: ${zl.zaehlart} (${zaehlartText.get(zl.zaehlart)})`)
    const zaehldauer = new TextAsset(`Zähldauer: ${zaehldauerText.get(zl.zaehldauer)}`)
    const zaehlsituation = new TextAsset(`Zählsituation/ erweiterte Zählsituation: ${this.checkForNull(zl.zaehlsituation)} / ${this.checkForNull(zl.zaehlsituationErweitert)}`)
    const wetter = new TextAsset(`Wetter: ${wetterText.get(zl.wetter)}`)
    const quelle = new TextAsset(`Quelle: ${quelleText.get(zl.quelle)}`)
    const kommentar = new TextAsset(`Zählungskommentar: ${this.checkForNull(zl.kommentar)}`)
    let knotenarmListe = "Knotenarme:<br/>"
    zl.knotenarme.forEach(k => {
      knotenarmListe = knotenarmListe + `${k.nummer} ${k.strassenname}<br/>\n`
    })
    const knotenarme = new TextAsset(knotenarmListe)
    this.$store.dispatch("addAssets", [headline, projektname, zaehlart, zaehldauer, zaehlsituation, wetter, quelle, kommentar, knotenarme])
  }
  ...
  createZaehlungskenngroessen() {
    ...
    const headline = new HeadingAsset(`Kenngrößen zur Zählung vom ${datum} (Zs-Nr. ${zs.nummer})`, AssetTypesEnum.HEADING3);
    const zaehlungskenngroessen = new ZaehlungskenngroessenAsset(`Zählungskenngrößen (Zählstelle ${zs.nummer}, ${datum})`, zl.id);
   ...
   */
  createLegende() {
    const ueberschrift = new HeadingAsset("Legende zu den Kennzahlen", AssetTypesEnum.HEADING3)
    const legende = new TextAsset("<p>- <b>Kraftfahrzeugverkehr (KFZ)</b>: Der Kraftfahrzeugverkehr ist die Summe der Personenkraftwagen, Krafträder, Busse, Lastkraftwagen und Lastzüge.</p>\n" +

<template>
  ...
  >
    ...
    -->
    <v-overlay value="true" opacity="0" z-index="-99999999">
      <belastungsplan-kreuzung-svg-schematische-uebersicht
          :dimension="contentHeight"
          :data="belastungsplanData"
          :doc-mode="false"
          :geometrie-mode="true"
          v-on:print="storeSvgSchematischeUebersicht($event)"
      ></belastungsplan-kreuzung-svg-schematische-uebersicht>
    </v-overlay>
    <v-banner
        single-line
    ...
    >
      ...
      Zu dieser Zählstelle sind aktuell noch keine Zählungen im System vorhanden.
    </v-banner>
    <v-tabs
        v-model="activeTab"
        fixed-tabs
    ...
    <v-speed-dial v-if="isTabListenausgabe"
                  v-model="fab"
                  absolute
                  bottom
                  right
    ...
    >
      ...
      <v-tooltip left v-if="!isTabHeatmap">
        <template v-slot:activator="{ on, attrs }">
          <v-btn
              fab
              dark
    ...
    <PdfReportMenue
        ...
        v-on:close="closePdfReportDialog"
    >
    </PdfReportMenue>
  </v-sheet>
</template>
...
export default class ZaehldatenDiagramme extends Vue {
  ...
  private belastungsplanLoading: boolean = false
  private chartDataLoading: boolean = false

  // Dialog zum PDF Report
  private pdfReportDialog: boolean = false

  // Belastungsplan Kreuzung
  private belastungsplanDTO: LadeBelastungsplanDTO = {} as LadeBelastungsplanDTO;
  private belastungsplanLoaded: boolean = false;
  private belastungsplanSvg: Blob | null = null;
  private belastungsplanPngBase64: string = '';
  private belastungsplanSchematischeUebersichtSvg: Blob | null = null;
  private belastungsplanSchematischeUebersichtPngBase64: string = '';


  // Stepline
  private zaehldatenSteplineDTO: LadeZaehldatenSteplineDTO = {} as LadeZaehldatenSteplineDTO;

  // Listenausgabe
  private listenausgabeDTO: Array<LadeZaehldatumDTO> = [];

  // Heatmap
  private zaehldatenHeatmap: LadeZaehldatenHeatmapDTO = {} as LadeZaehldatenHeatmapDTO;

  // Zeitreihe
  private zaehldatenZeitreihe: LadeZaehldatenZeitreiheDTO = {} as LadeZaehldatenZeitreiheDTO;
  private zeitreiheLoading: boolean = false;

  // Fab
  private fab: boolean = false;
  private isFabShown: boolean = true;
  private isTabListenausgabe: boolean = false;
  private isTabHeatmap: boolean = false;
  ...
  private static readonly REQUEST_PART_CHART_AS_BASE64_PNG: string = "chartAsBase64Png";
  private static readonly REQUEST_PART_SCHEMATISCHE_UEBERSICHT_AS_BASE64_PNG: string = "schematischeUebersichtAsBase64Png";
  ...
  changePdfFabVisibility(active: number): void {
    ...
    this.isTabListenausgabe = [ZaehldatenDiagramme.TAB_LISTENAUSGABE].includes(this.activeTab);
    this.isTabHeatmap = [ZaehldatenDiagramme.TAB_HEATMAP].includes(this.activeTab);
    if ([ZaehldatenDiagramme.TAB_BELASTUNGSPLAN, ZaehldatenDiagramme.TAB_GANGLINIE, ZaehldatenDiagramme.TAB_LISTENAUSGABE, ZaehldatenDiagramme.TAB_HEATMAP, ZaehldatenDiagramme.TAB_ZEITREIHE].includes(this.activeTab)) {
  ...
  storeBelastungsplanSvgAsPng(): void {
    if (this.belastungsplanSvg != null) {
      ...
      image.onload = () => {
        ...
        const dimension = ZaehldatenDiagramme.BELASTUNGSPLAN_PNG_DIMENSION
  ...
  storeBelastungsplanSchematischeUebersichtSvgAsPng(): void {
    if (this.belastungsplanSchematischeUebersichtSvg != null) {
      ...
      image.onload = () => {
        const canvas = document.createElement('canvas')
        const dimension = ZaehldatenDiagramme.BELASTUNGSPLAN_SCHEMATISCHE_UEBERSICHT_PNG_DIMENSION
        canvas.width = dimension
        canvas.height = dimension
        const context = canvas.getContext('2d')
        if (context) {
      ...
      image.src = URL.createObjectURL(this.belastungsplanSchematischeUebersichtSvg)
    }
  }
  ...
  private loadZeitreihe(options: OptionsDTO) {
    ...
    let zaehlstelle: ZaehlstelleHeaderDTO = this.$store.getters.getZaehlstelle;
  ...
  private loadBelastungsplan(options: OptionsDTO) {
    ...
    LadeZaehldatenService.ladeBelastungsplan(
        this.zaehlung.id,
        options).then((dto: LadeBelastungsplanDTO) => {
  ...
  private loadProcessedChartData(options: OptionsDTO) {
    ...
    LadeZaehldatenService.ladeZaehldatenProcessed(
        this.zaehlung.id,
        options).then((processedZaehldaten: LadeProcessedZaehldatenDTO) => {
      this.listenausgabeDTO = processedZaehldaten.zaehldatenTable.zaehldaten;
      this.zaehldatenSteplineDTO = processedZaehldaten.zaehldatenStepline;
      this.zaehldatenHeatmap = processedZaehldaten.zaehldatenHeatmap;
      this.setMaxRangeYAchse();
    }).finally(() => {
      this.chartDataLoading = false;
      this.storeStartAndEndeUhrzeitOfIntervalls(this.listenausgabeData);
    })
  }

  private setMaxRangeYAchse() {
    let ganglinieYAchse1MaxValue: number | null = this.options.ganglinieYAchse1MaxValue;
    if (ganglinieYAchse1MaxValue !== undefined && ganglinieYAchse1MaxValue !== null && ganglinieYAchse1MaxValue > 0) {
      this.zaehldatenSteplineDTO.rangeMax = ganglinieYAchse1MaxValue;
    }

    let ganglinieYAchse2MaxValue: number | null = this.options.ganglinieYAchse2MaxValue;
    if (ganglinieYAchse2MaxValue !== undefined && ganglinieYAchse2MaxValue !== null && ganglinieYAchse2MaxValue > 0) {
      this.zaehldatenSteplineDTO.rangeMaxPercent = ganglinieYAchse2MaxValue;
    }
  }
  ...
  addChartToPdfReport(): void {
    ...
    if (this.activeTab === ZaehldatenDiagramme.TAB_BELASTUNGSPLAN) {
      ...
      if (this.belastungsplanDTO.kreisverkehr) {
        // Kreisverkehr
        this.addImageToReport(this.getKreisverkehrBase64(), this.createCaption("Belastungsplan"), false)
      } else {
        // Kreuzung
        this.addImageToReport(this.belastungsplanPngBase64, this.createCaption("Belastungsplan"), false)
      }
    ...
    if (this.activeTab === ZaehldatenDiagramme.TAB_GANGLINIE) {
      ...
      this.addImageToReport(this.getGanglinieBase64(), this.createCaption("Ganglinie"), true)
    }
    // Heatmap
    if (this.activeTab === ZaehldatenDiagramme.TAB_HEATMAP) {
      type = "Die Heatmap"
      this.addImageToReport(this.getHeatmapBase64(), this.createCaption("Heatmap"), true)
    }
    // Zeitreihe
    if (this.activeTab === ZaehldatenDiagramme.TAB_ZEITREIHE) {
      type = "Die Zeitreihe"
      this.addImageToReport(this.getZeitreiheBase64(), this.createCaption("Zeitreihe"), true)
    }
    // Listenausgabe
    if (this.activeTab === ZaehldatenDiagramme.TAB_LISTENAUSGABE) {
      type = "Die Datentabelle"
      this.addHeadingToReport();
      const datatableAsset: DatatableAsset = new DatatableAsset(Object.assign({}, this.options), this.zaehlungsId, this.createCaption("Datentabelle"));
  ...
  createCaption(diagram: string): string {
    const zdate = this.$d(Date.parse(this.selectedZaehlung.datum), 'short', 'de-DE')
  ...
  addHeadingToReport(): void {
    ...
    if(chartTitle !== "") {
      const headingAsset = new HeadingAsset(chartTitle, AssetTypesEnum.HEADING5);
  ...
  private generatePdf() {
    ...
    formData.append("options", new Blob([JSON.stringify(o)], {
    ...
    if (this.activeTab === ZaehldatenDiagramme.TAB_BELASTUNGSPLAN) {
      // Kreisverkehr
      if (this.belastungsplanDTO.kreisverkehr) {
        formData.append(ZaehldatenDiagramme.REQUEST_PART_CHART_AS_BASE64_PNG, new Blob([this.getKreisverkehrBase64()], {
          type: "image/png"
        }));
      } else {
        // Kreuzung
        formData.append(ZaehldatenDiagramme.REQUEST_PART_CHART_AS_BASE64_PNG, this.belastungsplanPngBase64);
      }
      this.fetchPdf(formData, "belastungsplan");

      // Ganglinie
    } else if (this.activeTab === ZaehldatenDiagramme.TAB_GANGLINIE) {
      formData.append(ZaehldatenDiagramme.REQUEST_PART_CHART_AS_BASE64_PNG, new Blob([this.getGanglinieBase64()], {
        type: "image/png"
      }));
      formData.append(ZaehldatenDiagramme.REQUEST_PART_SCHEMATISCHE_UEBERSICHT_AS_BASE64_PNG, this.belastungsplanSchematischeUebersichtPngBase64);
      ...
      formData.append(ZaehldatenDiagramme.REQUEST_PART_CHART_AS_BASE64_PNG, new Blob([this.getZeitreiheBase64()], {
        type: "image/png"
      }));
      formData.append(ZaehldatenDiagramme.REQUEST_PART_SCHEMATISCHE_UEBERSICHT_AS_BASE64_PNG, this.belastungsplanSchematischeUebersichtPngBase64);
  ...
  private fetchPdf(formData: any, type: string) {
    ...
    GeneratePdfService.postPdfCustomFetchTemplate(type, this.zaehlung.id, formData).then(res => {
      res.blob()
          .then(blob => {
            // Erster Buchstabe soll im Dateinamen groß geschrieben sein, also z. B. Ganglinie statt ganglinie.
            let typeForFilename: string = type.charAt(0).toUpperCase() + type.slice(1);

            // Beispiel: 251101K_15-11-2020_Belastungsplan.pdf
            let filename: string = `${this.getFileName()}_${typeForFilename}.pdf`
  ...
  private generateCsv() {
    ...
    GenerateCsvService.generateCsv(this.zaehlung.id, optionsDTO)
        .then((result: CsvDTO) => {
          ...
          let filename: string = `${this.getFileName()}_Listenausgabe.csv`
  ...
  private getFileName(): string {
    ...
    let dateForFilename: string = new Date(this.zaehlung.datum).toISOString().split('T')[0];
   ...
   */
  private storeStartAndEndeUhrzeitOfIntervalls(listenausgabeData: Array<LadeZaehldatumDTO>): void {
    const intervalls: Array<LadeZaehldatumDTO> = listenausgabeData.filter(zaehldatum => {
    ...
    if (!_.isEmpty(firstIntervall) && !_.isEmpty(lastIntervall)) {
      ...
      this.$store.commit("setStartEndeUhrzeitIntervalls", startEndeUhrzeitIntervalls);
    }
  }

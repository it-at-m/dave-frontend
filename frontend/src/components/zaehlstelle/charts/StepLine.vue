...
export default class StepLine extends Vue {
  ...
  private twoChartsNeeded: boolean = false;

  private chartLegend: Array<string> = [];

  private rangeMax: number = 0;

  private rangeMaxPercent: number = 0;
  ...
  private setDataForChart(zaehldatenStepline: LadeZaehldatenSteplineDTO) {
    ...
    if (zaehldatenStepline.seriesEntriesSecondChart !== null) {
      this.seriesEntriesChart = this.createSeriesDataForChart(
          [...zaehldatenStepline.seriesEntriesFirstChart, ...zaehldatenStepline.seriesEntriesSecondChart]
      );
    } else {
      this.seriesEntriesChart = this.createSeriesDataForChart(zaehldatenStepline.seriesEntriesFirstChart);
    }
  }

  private checkTwoChartsNeeded(zaehldatenStepline: LadeZaehldatenSteplineDTO): boolean {
    return zaehldatenStepline.xaxisDataSecondChart !== null
        && zaehldatenStepline.seriesEntriesSecondChart !== null;
  }

  private createSeriesDataForChart(seriesEntriesChart: Array<StepLineSeriesEntryDTO>): any[] {
    let seriesEntries: any[] = [];
    seriesEntriesChart.forEach(seriesEntryChart => {
      if (seriesEntryChart.name === StepLine.LEGEND_ENTRY_GV_ANTEIL_PROZENT
          || seriesEntryChart.name === StepLine.LEGEND_ENTRY_SV_ANTEIL_PROZENT) {
        seriesEntries.push({

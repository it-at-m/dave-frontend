...
export default class Zeitreihe extends Vue {
    ...
    let options = {
      tooltip: {
        ...
        formatter: function (params: any[]) {
          ...
          params.forEach(value => {
            if (value.seriesName == Zeitreihe.SCHWERVERKEHRSANTEIL || value.seriesName == Zeitreihe.GUETERVERKEHRSANTEIL) {
              text += `<span style="color:${value.color}">\u25CF</span> ${value.seriesName}: ${value.data.toLocaleString()}%<br/>`;
            } else {
              text += `<span style="color:${value.color}">\u25CF</span> ${value.seriesName}: ${value.data.toLocaleString()}<br/>`;
            }
          })
      ...
      toolbox: {
        ...
        feature: {
          ...
          dataView: {
            show: true, readOnly: true, title: 'Datenansicht', lang: ['Datenansicht', 'zurück', 'refresh'],
      ...
      legend: {
        data: [Zeitreihe.KRAFTFAHRZEUGVERKEHR, Zeitreihe.GUETERVERKEHR, Zeitreihe.SCHWERVERKEHR, Zeitreihe.RADVERKEHR, Zeitreihe.FUSSVERKEHR, Zeitreihe.GESAMT, Zeitreihe.SCHWERVERKEHRSANTEIL, Zeitreihe.GUETERVERKEHRSANTEIL],
        selectedMode: false,
      },
      xAxis: {
        type: 'category',
  ...
  get secondYAxis() {
    if (this.filterOptions.schwerverkehrsanteilProzent || this.filterOptions.gueterverkehrsanteilProzent) {
      return {
        type: 'value',
        name: 'in Prozent',
        min: 0,
  ...
  private createSeriesEntries(zeitreiheDaten: LadeZaehldatenZeitreiheDTO) {
    ...
    if (this.filterOptions.schwerverkehrsanteilProzent) {
      series.push({
        ...
        color: StepLine.CHART_COLOR.get(StepLine.LEGEND_ENTRY_SV_ANTEIL_PROZENT),
      });
    }
    ...
    if (this.filterOptions.gueterverkehrsanteilProzent) {
      series.push({
        ...
        color: StepLine.CHART_COLOR.get(StepLine.LEGEND_ENTRY_GV_ANTEIL_PROZENT),
      });
    }
  ...
  private downloadCsv() {
    const header = 'Zähldatum;Kraftfahrzeugverkehr;Güterverkehr;Schwerverkehr;Radverkehr;Fussverkehr;Gesamt;Schwerverkehrsanteil;Güterverkehrsanteil'
    const rows = [];

    rows.push(header);

    for (let index = 0; index < this.zeitreiheDaten.svAnteilInProzent.length; index++) {
      let row: string = "";
      row += `${this.zeitreiheDaten.datum[index]}`;
      row += this.fillCsvRow(this.filterOptions.kraftfahrzeugverkehr, this.zeitreiheDaten.kfz[index])
      row += this.fillCsvRow(this.filterOptions.gueterverkehr, this.zeitreiheDaten.gv[index])
      row += this.fillCsvRow(this.filterOptions.schwerverkehr, this.zeitreiheDaten.sv[index])
      row += this.fillCsvRow(this.filterOptions.radverkehr, this.zeitreiheDaten.rad[index])
      row += this.fillCsvRow(this.filterOptions.fussverkehr, this.zeitreiheDaten.fuss[index])
      row += this.fillCsvRow(this.filterOptions.zeitreiheGesamt, this.zeitreiheDaten.gesamt[index])
      row += this.fillCsvRow(this.filterOptions.schwerverkehrsanteilProzent, this.zeitreiheDaten.svAnteilInProzent[index])
      row += this.fillCsvRow(this.filterOptions.gueterverkehrsanteilProzent, this.zeitreiheDaten.gvAnteilInProzent[index])
      rows.push(row);
    }

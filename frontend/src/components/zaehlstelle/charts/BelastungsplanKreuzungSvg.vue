...
export default class BelastungsplanKreuzungSvg extends Vue {
  ...
  mounted() {
    this.canvas = SVG.SVG().addTo('#belastungsplan').size(this.dimension, this.dimension).viewbox(0, 0, this.viewbox, this.viewbox);
    this.$store.dispatch("setSizeBelastungsplanSvg", this.sizeBelastungsplan.replace('px', ''));
    this.$store.dispatch("setMaxSizeBelastungsplanSvg", this.maxSizeBelastungsplan);
    this.$store.dispatch("setMinSizeBelastungsplanSvg", this.minSizeBelastungsplan);
  }
  ...
  draw() {
    ...
    this.documentationGroup.rect(this.viewbox, this.viewbox).fill({color: "white", opacity: this.docMode ? 0.3 : 0.0})
    ...
    if (knotenarme) {
      ...
      cleanseMap.forEach((group, knotenarmnummer) => {
        ...
        const m = new BerechnungsMatrix(matrix.a, matrix.b, matrix.c, matrix.d, matrix.e, matrix.f)
    ...
    const ex = this.canvas.flatten(this.canvas).size(size, size).svg() as string
    this.$emit("print", new Blob([ex], {type: "image/svg+xml;charset=utf-8"}))
  }
   ...
   */
  recreatePath(svgMatrix: BerechnungsMatrix, pathArray: SVG.PathArray): string {
    let path = ""
   ...
   */
  lineColor(vonKnotenarm: number, nachKnotenarm: number, vonWert: number): string {
    ...
    if (this.geometrieMode && this.vonIds.length > 0) {
      if (!(this.vonIds.includes(vonKnotenarm) && this.nachIds.includes(nachKnotenarm))) {
        if (!(this.beideRichtungen && this.vonIds.includes(nachKnotenarm) && this.nachIds.includes(vonKnotenarm))) {
  ...
  fahrtrichtungVon(knotenarmnummer: number): SVG.G {
    ...
    if (this.fahrbeziehungsTypen.has(knotenarmnummer) && knotenarm) {
      ...
      fahrbeziehungsGroup.rect(this.viewbox, this.viewbox).fill({color: "white", opacity: opacity})
      // hier werden die Schattenlinien der Fahrbeziehungen je Knotenarm gesammelt
      const shadowGroup = canvas.group() as SVG.G
      shadowGroup.rect(this.viewbox, this.viewbox).fill({color: "white", opacity: opacity})
      shadowGroup.back()
      ...
      let fbts = this.fahrbeziehungsTypen
          .get(knotenarmnummer)?.sort(BelastungsplanFahrbeziehungComperator.sortByFahrtrichtungsart) as BelastungsplanFahrbeziehung[]
      ...
      fbts.forEach(fbt => {
        ...
        const vonPosition = this.knotenarme.get(knotenarmnummer)!.findFahrbeziehungVonPosition(fbt.fahrbeziehungsTyp)

        // Die Position der Fahrbeziehung in der Fahrtrichtung "nach"
        let nachPosition = this.knotenarme.get(fbt.nach)!.findFahrbeziehungsNachPosition(fbt.fahrbeziehungsTyp)

        // Der Startpunkt
        const startX = this.chartPosition
        const startY = this.viewbox / 2 + this.basis / 2 - vonPosition * line

        // rechts 45°
        if (fbt.fahrbeziehungsTyp === Fahrtrichtungsarten.RECHTS45) {
          const start = new SVG.Point(startX, startY)
          const achteck1 = new SVG.Point(this.chartPosition, this.chartPosition + this.ecke + this.calcA())
          const achteck2 = new SVG.Point(this.chartPosition + this.ecke + this.calcA(), this.chartPosition)
          const path = this.zielPunkt(
              achteck1,
              new SVG.Point(achteck1.x + this.ecke, achteck1.y + this.ecke),
              achteck2,
              new SVG.Point(achteck2.x + this.ecke, achteck2.y + this.ecke),
              start,
              nachPosition,
              this.ecke / 4,
              true
          )
          fahrbeziehungsGroup.path(path)
              .stroke({
                width: this.calcLineWidth(fbt.total),
                color: this.lineColor(knotenarmnummer, fbt.nach, fbt.total)
              })
              .attr("fill", "none")
          shadowGroup.path(path)
              .stroke({width: this.lineWidth, color: this.shadowColor, opacity: this.shadowOpacity})
              .attr("fill", "none")
        }

        // rechts 90°
        if (fbt.fahrbeziehungsTyp === Fahrtrichtungsarten.RECHTS90) {
          const zielX = startX + this.ecke + this.spalt + nachPosition * this.line
          const zielY = this.viewbox - this.chartPosition
          const path = `M${startX} ${startY}
                        L${startX + this.ecke + nachPosition * this.line} ${startY}
                        Q${zielX} ${startY}
        ...
        if (fbt.fahrbeziehungsTyp === Fahrtrichtungsarten.RECHTS135) {
          const start = new SVG.Point(startX, startY)
          const achteck1 = new SVG.Point(this.chartPosition + this.ecke + this.calcA(), this.chartPosition + this.seite)
          const achteck2 = new SVG.Point(this.chartPosition, this.chartPosition + this.ecke)
          const path = this.zielPunkt(
              achteck1,
              new SVG.Point(achteck1.x + this.ecke, achteck1.y - this.ecke),
              achteck2,
        ...
        if (fbt.fahrbeziehungsTyp === Fahrtrichtungsarten.GERADE) {
          const path = `M${startX} ${startY} L${startX + this.seite} ${startY}`
          fahrbeziehungsGroup.path(path).stroke({
            width: this.calcLineWidth(fbt.total),
            color: this.lineColor(knotenarmnummer, fbt.nach, fbt.total)
          })
          shadowGroup.path(path).stroke({width: this.lineWidth, color: this.shadowColor, opacity: this.shadowOpacity})
        }

        // links 45°
        if (fbt.fahrbeziehungsTyp === Fahrtrichtungsarten.LINKS45) {
          const start = new SVG.Point(startX, startY)
          const achteck1 = new SVG.Point(this.chartPosition + this.ecke, this.chartPosition)
          const achteck2 = new SVG.Point(this.chartPosition + this.seite, this.chartPosition + this.ecke + this.calcA())
          const path = this.zielPunkt(
              achteck1,
              new SVG.Point(achteck1.x - this.ecke, achteck1.y + this.ecke),
              achteck2,
        ...
        if (fbt.fahrbeziehungsTyp === Fahrtrichtungsarten.LINKS90) {
          const zielX = startX + this.seite - this.ecke - this.spalt - nachPosition * line
          const path = `M${startX} ${startY}
                        L${startX + this.seite / 2 - this.seite * 0.07} ${startY}
                        Q${zielX} ${startY}
        ...
        if (fbt.fahrbeziehungsTyp === Fahrtrichtungsarten.LINKS135) {
          const start = new SVG.Point(startX, startY)
          const achteck1 = new SVG.Point(this.chartPosition + this.seite, this.chartPosition + this.ecke)
          const achteck2 = new SVG.Point(this.chartPosition + this.ecke, this.chartPosition + this.seite)
          const path = this.zielPunkt(
              achteck1,
              new SVG.Point(achteck1.x - this.ecke, achteck1.y - this.ecke),
              achteck2,
        ...
        if (fbt.fahrbeziehungsTyp === Fahrtrichtungsarten.UTURN) {
          ...
          const zielY = this.chartPosition + this.ecke + this.spalt + nachPosition * line
          const path = `M${startX} ${startY}
                        L${wendeX} ${startY}
                        Q${querX} ${startY}
      ...
      // Labels
      // Spaltenbreite
      const colWidth = Math.ceil((this.chartPosition - this.lineGap * 5 - 15) / 3)
      ...
      fahrbeziehungenLabelRotationGroup.rect(this.viewbox, this.viewbox).fill({color: "white", opacity: opacity})
      ...
      let yNach = this.chartPosition + this.ecke + this.spalt + this.basis - (knotenarm.anzahlNachFahrbeziehungen * this.lineWidth) / 2 - this.lineGap - ausgleich
      let yD1 = this.chartPosition + this.ecke + this.spalt + this.basis + line / 2 - ausgleich
      let yD2 = this.chartPosition + this.ecke + this.spalt + this.basis - (knotenarm.anzahlNachFahrbeziehungen * this.lineWidth) + this.lineGap - ausgleich
      let yVonSumme = this.chartPosition + this.ecke + this.spalt + knotenarm.anzahlVonFahrbeziehungen * this.line - this.lineGap * 2
      let yVon = this.chartPosition + this.ecke + this.spalt
      let yKnotenarm = this.chartPosition + this.seite / 2 - this.line / 2
      let xText = (this.chartPosition + this.seite) - (this.ecke - this.spalt * 1.5) / 2 + 5
      ...
      if ([3, 4, 7, 8].includes(knotenarmnummer)) {
        yNach = this.chartPosition + this.ecke + this.spalt + (knotenarm.anzahlNachFahrbeziehungen * this.lineWidth) / 2 - this.lineWidth + this.lineGap + ausgleich
        yD1 = this.chartPosition + this.ecke + this.spalt - this.lineWidth / 2 + ausgleich
        yD2 = this.chartPosition + this.ecke + this.spalt + (knotenarm.anzahlNachFahrbeziehungen * this.lineWidth) + ausgleich
        yVonSumme = this.chartPosition + this.ecke + this.spalt + this.basis + this.lineWidth / 2 + this.lineGap
        yVon = this.chartPosition + this.ecke + this.spalt + this.basis - (knotenarm.anzahlVonFahrbeziehungen * line) + line
        xc1 = colWidth
        xc2 = colWidth * 2
        xc3 = colWidth * 3
        xD1 = xc3 + this.chartPosition - colWidth * 3 - this.lineGap
        xD2 = xD1 - 20
        xText = this.chartPosition + (this.ecke - this.spalt * 1.5) / 2 - 5
        links = true
      }

      // Knotenarmname
      this.createKnotenarmname(fahrbeziehungenLabelRotationGroup, knotenarm, xText, yKnotenarm)

      // Knotenarmsumme
      this.labelZeileErstellen(
          fahrbeziehungenLabelRotationGroup,
          yKnotenarm,
          this.fillValueArray(knotenarm, [knotenarm.totalValue1, knotenarm.totalValue2, knotenarm.totalValue3]),
      ...
      if (knotenarm.anzahlNachFahrbeziehungen > 0) {
        // Dreieck
        const dreieck = `M${xD1} ${yD1} L${xD2} ${yNach + line / 2} L${xD1} ${yD2} z`
        fahrbeziehungenLabelRotationGroup.path(dreieck)
            .stroke({width: 2, color: "black"})
            .attr("fill", "none")
        ...
        this.labelZeileErstellen(
            fahrbeziehungenLabelRotationGroup,
            yNach,
            this.fillValueArray(knotenarm, [knotenarm.nachTotalValue1, knotenarm.nachTotalValue2, knotenarm.nachTotalValue3]),
            this.calculateLabelPosition(knotenarm, [xc1, xc2, xc3], links),
            "bold"
        )
      }
      ...
      fbts.forEach(fbt => {
        ...
        this.labelZeileErstellen(
            fahrbeziehungenLabelRotationGroup,
            y,
            this.fillValueArray(knotenarm, [fbt.value1, fbt.value2, fbt.value3]),
            this.calculateLabelPosition(knotenarm, [xc1, xc2, xc3], links)
        )
      ...
      if (knotenarm.anzahlVonFahrbeziehungen > 0) {
        this.labelZeileErstellen(
            fahrbeziehungenLabelRotationGroup,
            yVonSumme,
            this.fillValueArray(knotenarm, [knotenarm.vonTotalValue1, knotenarm.vonTotalValue2, knotenarm.vonTotalValue3]),
            this.calculateLabelPosition(knotenarm, [xc1, xc2, xc3], links),
            "bold"
        )
        // Summenlinie
      ...
      fahrbeziehungenLabelRotationGroup.rotate(this.calcLabelRotation(knotenarmnummer))

    } else {

      if (!this.fahrbeziehungsTypen.has(knotenarmnummer)) {
        console.warn(`Für die Knotenarmnummer ${knotenarmnummer} wurden keine Fahrbeziehungen gefunden!`, this.fahrbeziehungsTypen)
      }

      if (!knotenarm) {
        console.warn(`Für die Knotenarmnummer ${knotenarmnummer} liegt kein Knotenarm vor!`, this.knotenarme)
      }
    }
    return knotenarmGroup
  }

  calculateLabelPosition(knotenarm: BelastungsplanKnotenarm, werte: number[], links: boolean): number[] {
    if (links) {
   ...
   */
  fillValueArray(knotenarm: BelastungsplanKnotenarm, werte: number []): number[] {
   ...
   * @param y           Y-Position
   */
  createKnotenarmname(group: SVG.G, knotenarm: BelastungsplanKnotenarm, x: number, y: number) {
    ...
    group.text(add => {
      if (pieces.length > 1) {
        ...
        const firstAndSecondLineOfStreetnames: Array<string> = this.getFirstAndSecondLineOfStreetname(trenner, pieces);
        add.tspan(firstAndSecondLineOfStreetnames[0]).x(x).dy(this.line)
        add.tspan(firstAndSecondLineOfStreetnames[1]).x(x).dy(this.line)
  ...
  legendeZaehlstellenInfo() {
    ...
    info.text(add => {
      add.tspan(`Zählstelle ${this.zaehlstelle.nummer}`).font({weight: "bold"})
      add.tspan(`Stadtbezirk ${this.zaehlstelle.stadtbezirkNummer}`).newLine()
      add.tspan(`Zähldatum: ${this.$d(Date.parse(this.zaehlung.datum), 'short', 'de-DE')}`).newLine()
  ...
  legendeSpalten() {
    ...
    if (this.isDifferenzdatendarstellung) {
      ...
      formeln.set("RAD (KI-Hochrechnung)", "RAD-KI = RAD-KI (Basis) - RAD-KI (Vergleich)")
    ...
    let zaehlzeitSecondLine: string = '';
    if (zeitauswahl === Zeitauswahl.TAGESWERT) {
      zaehlzeitSecondLine = `${this.zaehlung.zaehldauer === Zaehldauer.DAUER_24_STUNDEN ? zeitblockInfo.get(Zeitblock.ZB_00_24)?.text : 'hochgerechnet'}`
    } else if (zeitauswahl === Zeitauswahl.BLOCK) {
      zaehlzeitSecondLine = `${zeitblockInfo.get(this.optionen.zeitblock)?.text}`
    } else if (zeitauswahl === Zeitauswahl.STUNDE) {
      zaehlzeitSecondLine = `${zeitblockStuendlichInfo.get(this.optionen.zeitblock)?.text}`
    } else if (zeitauswahl === Zeitauswahl.SPITZENSTUNDE_KFZ || zeitauswahl === Zeitauswahl.SPITZENSTUNDE_RAD || zeitauswahl === Zeitauswahl.SPITZENSTUNDE_FUSS) {
      const startEndeUhrzeitIntervalls: StartEndeUhrzeitIntervalls = this.$store.getters.getStartEndeUhrzeitIntervalls;
    ...
    spalten.text(add => {
      ...
      add.tspan(`${this.data.value1.label}`).font({weight: "bold"}).newLine().x(startX)
      // Datenblock 2 gefüllt?
      if (this.data.value2.filled) {
        add.tspan(`(${this.data.value2.label})`).font({weight: "bold"}).x(startX + 60)
      }
      // Datenblock 3 gefüllt?
      if (this.data.value3.filled) {
        add.tspan(`${this.data.value3.label}`).font({weight: "bold"}).x(startX + 120)
      }

      // Formeln
      add.tspan(`${formeln.get(this.data.value1.label)}`).newLine().x(startX)
      // Datenblock 2 gefüllt?
      if (this.data.value2.filled) {
        add.tspan(`${formeln.get(this.data.value2.label)}`).newLine().x(startX)
      }
      // Datenblock 3 gefüllt?
      if (this.data.value3.filled) {
        add.tspan(`${formeln.get(this.data.value3.label)}`).newLine().x(startX)
      }
    })
        .font({
          family: this.fontfamily,
  ...
  get getLegendLineWidth(): number {
    ...
    if (this.data && this.data.value1.filled && this.data.value2.filled && this.data.value3.filled) {
  ...
  legendeLinienStaerke() {
    ...
    // grün = Abnahme
    if (this.isDifferenzdatendarstellung) {
      size.rect(this.lineWidth, this.lineWidth).fill(this.zunahmeValueColor).x(startX).y(startY + 35)
      size.rect(this.lineWidth, this.lineWidth).fill(this.abnahmeValueColor).x(startX).y(startY + 35 + 5 + this.lineWidth)
    ...
    const path2 = `M${startX + 1.5 * this.spalt} ${startY}
                   L${startX + 1.5 * this.spalt} ${startY - this.maxlineWidth / 2}`
    size.path(path2)
        .stroke({width: 2, color: this.legendColor})
        .attr("fill", "none")

    const high = this.highestFahrbeziehungsValue + (100 - this.highestFahrbeziehungsValue % 1000)
    size.text(add => {
      add.tspan(`${high}`)
    ...
    if (fbs && Array.isArray(fbs)) {
      ...
      fbs.forEach(fb => {
        ...
        if (!this.fahrbeziehungsTypen.has(fb.von)) {
          this.fahrbeziehungsTypen.set(fb.von, new Array<BelastungsplanFahrbeziehung>())
        }

        // Für Knotenarme, die nur eingehende Fahrbeziehungen haben.
        if (!this.fahrbeziehungsTypen.has(fb.nach)) {
          this.fahrbeziehungsTypen.set(fb.nach, new Array<BelastungsplanFahrbeziehung>())
        }
        ...
        if (!this.knotenarme.has(fb.von)) {
          this.knotenarme.set(fb.von, new BelastungsplanKnotenarm(data.streets[fb.von - 1], fb.von))
        }
        ...
        if (!this.knotenarme.has(fb.nach)) {
          this.knotenarme.set(fb.nach, new BelastungsplanKnotenarm((data.streets[fb.nach - 1]), fb.nach))
        }
        ...
        let belastungsplanFahrbeziehungen = this.fahrbeziehungsTypen.get(fb.von) as BelastungsplanFahrbeziehung[]
        let belastungsplanFahrbeziehung = new BelastungsplanFahrbeziehung(this.calcFahrbeziehungstype(fb.von, fb.nach), fb.nach) as BelastungsplanFahrbeziehung
        ...
        if (knotenarmVon) {
          ...
          if (this.positiveNumber(knotenarmVon.anzahlVonFahrbeziehungen) > this.vonMaxValue)
            this.vonMaxValue = this.positiveNumber(knotenarmVon.anzahlVonFahrbeziehungen)
          // den höchsten und niedrigsten Wert einer Fahrbeziehung ermitteln
          // (gilt hier nur der KFZ Verkehr?)
          if (this.positiveNumber(data.value1.values[v][n]) > this.highestFahrbeziehungsValue) this.highestFahrbeziehungsValue = this.positiveNumber(data.value1.values[v][n])
          if (this.positiveNumber(data.value1.values[v][n]) < this.lowestFahrbeziehungsValue) this.lowestFahrbeziehungsValue = this.positiveNumber(data.value1.values[v][n])
          // Fahrbeziehungstyp wird gesetzt um später die Position der Linien berrechnen zu können
          knotenarmVon.addVonFahrbeziehungsType(belastungsplanFahrbeziehung.fahrbeziehungsTyp)
        }

        if (knotenarmNach) {
          // nach Zähler hoch setzen
          knotenarmNach.plusFahrbeziehungenNach()
          if (this.positiveNumber(knotenarmNach.anzahlNachFahrbeziehungen) > this.nachMaxValue)
            this.nachMaxValue = this.positiveNumber(knotenarmNach.anzahlNachFahrbeziehungen)
          // den Typ der eingehenden Verbindung speichern
          knotenarmNach.addNachFahrbeziehungsTyp(belastungsplanFahrbeziehung.fahrbeziehungsTyp)
        }
      })
      ...
      this.knotenarme.forEach(k => {
        ...
        if (this.knotenarme.has(gegenueber)) {
          this.fahrbeziehungenAusrichten(k, this.knotenarme.get(gegenueber)!)
        }
        ...
        if (data.value2.filled && !(this.isDifferenzdatendarstellung && data.value2.percent)) {
        ...
        if (data.value3.filled && !(this.isDifferenzdatendarstellung && data.value3.percent)) {
   ...
   */
  anzeigeWerte(knotenarm: BelastungsplanKnotenarm | undefined, data: LadeBelastungsplanDTO) {
    if (knotenarm) {
   ...
   */
  fahrbeziehungenAusrichten(von: BelastungsplanKnotenarm, nach: BelastungsplanKnotenarm) {
    if (nach.nachFahrbeziehungsTypen.includes(Fahrtrichtungsarten.GERADE) && this.isGegenueber(von.knotenarmNummer, nach.knotenarmNummer)) {
   ...
   * @param nach  "nach" Knotenarm
   */
  ausgleichBerechnen(von: BelastungsplanKnotenarm, nach: BelastungsplanKnotenarm) {
    // Der Index der Geraden Fahrbeziehung muss voneinander abweichen, um ein Problem zu sein.
    const indexVon = von.vonFahrbeziehungsTypen.sort().indexOf(Fahrtrichtungsarten.GERADE)
    const indexNach = nach.findFahrbeziehungsNachPosition(Fahrtrichtungsarten.GERADE)
  ...
  calcMaxLineWidth() {
    ...
    let mv = 0
    this.vonMaxValue > this.nachMaxValue ? mv = this.vonMaxValue : mv = this.nachMaxValue
    if ((mv * mw) > this.maxFahrtrichtungWidth) {
      ...
      this.lineWidth = Math.ceil((this.maxFahrtrichtungWidth - mv * this.lineGap) / mv)
  ...
  calcFahrbeziehungstype(von: number, nach: number): number {
    ...
    // rechts 45°
    if (([2, 3, 4].includes(von) && (von + 3) === nach) ||
    ...
    // rechts 90°
    if (([4, 3, 2, 6, 7, 8].includes(von) && ((von - 1) === nach)) ||
        (von === 1 && nach === 4) ||
        (von === 5 && nach === 8)) {
    ...
    // links 45°
    if (([1, 2, 3, 4].includes(von) && (von + 4) === nach) ||
    ...
    // links 90°
    if ([1, 2, 3, 5, 6, 7].includes(von) && ((von + 1) === nach) ||
        (von === 4 && nach === 1) ||
        (von === 8 && nach === 5)) {
    ...
    // links 135°
    if (([1, 2, 3].includes(von) && (von + 5) === nach) ||
        (von === 4 && nach === 5) ||
    ...
    console.warn("Die Fahrtrichtungen konnten keine Abbiegeart zugeordnet werden: " + von + " -> " + nach)
    return -1
  }
  ...
  zielPunkt(achteck1: SVG.Point,
    ...
    // Zielgerade ziehen
    const pathZiel = this.documentationGroup
    ...
    const pathStart = this.documentationGroup
        .path(`M${startX1} ${startY1}
               L${startX2} ${startY2}`)
    ...
    if (this.docMode && doc) {
      ...
      this.documentationGroup.circle(10).fill({color: 'green'}).move(point1.x - 5, point1.y - 5)
      this.documentationGroup.circle(10).fill({color: 'green'}).move(startX1 - 5, startY1 - 5)
      this.documentationGroup.circle(6).fill({color: '#f06'}).move(point2.x - 3, point2.y - 3)
      this.documentationGroup.circle(6).fill({color: '#f06'}).move(startX2 - 3, startY2 - 3)
      this.documentationGroup.circle(10).fill({color: 'green'}).move(sx - 5, sy - 5)
      this.documentationGroup.circle(10).fill({color: 'green'}).move(startKurvenPunkt.x - 5, startKurvenPunkt.y - 5)
      this.documentationGroup.circle(10).fill({color: 'green'}).move(zielKurvenPunkt.x - 5, zielKurvenPunkt.y - 5)
    }
    return path
  }

  get isDifferenzdatendarstellung(): boolean {
    const differenzdaten = this.$store.getters.isDifferenzdatenDarstellung as boolean
  ...
  get vergleichsZaehlung(): LadeZaehlungDTO | undefined {
    if (this.isDifferenzdatendarstellung) {
      ...
      const zaehlung = this.$store.getters.getZaehlungById(vergleichsId) as LadeZaehlungDTO
      return zaehlung
    }
    console.warn(`Es wurde keine Differenzdatendarstellung ausgewählt. Entsprechend kann auch keine Vergleichszählung zurück geliefert werden.`)
    return undefined
  }
  ...
  get sizeBelastungsplan(): string {
    let sizeBelastungsplanSvg: number = this.$store.getters.getSizeBelastungsplanSvg;
  ...
  get minSizeBelastungsplan(): number {
    return (parseInt(this.dimension.trim().replace('vh', '')) * this.$vuetify.breakpoint.height) / 100;
  }

  get maxSizeBelastungsplan(): number {
    return (parseInt(this.dimension.trim().replace('vh', '')) * this.$vuetify.breakpoint.width) / 100;
  }
   ...
   */
  private getFirstAndSecondLineOfStreetname(trenner: string, pieces: Array<string>): Array<string> {
    ...
    const streetnames: Array<string> = this.getStreetnames(index, trenner, pieces);
  ...
  private getStreetnames(index: number, trenner: string, pieces: Array<string>): Array<string> {
    let firstLine: string = "";
    let secondLine: string = "";

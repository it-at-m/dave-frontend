<template>
  <v-card
      elevation="0"
  >
    <belastungsplan-kreisverkehr v-if="isKreisverkehr"
...
export default class BelastungsplanCard extends Vue {
  ...
  @Ref('belastungsplanKreisverkehr') readonly belastungsplanKreisverkehr!: BelastungsplanKreisverkehr;
  ...
  get visibleNodes(): boolean[] {
    let nodes: boolean[] = [];
    for (let index: number = 0; index < this.streets.length; index++) {

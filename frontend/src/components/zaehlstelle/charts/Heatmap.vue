...
export default class Heatmap extends Vue {
  ...
  get gridHeight() {
    let gridHeight: string = '50%';
    if (this.chartLegend.length !== 0) {
      gridHeight = this.chartLegend.length * Heatmap.HEIGHT_PERCENTAGE_SINGLE_ENTRY_Y_AXIS + '%';
    }
    return gridHeight;
  }
  ...
  private checkTwoChartsNeeded(zaehldatenHeatmap: LadeZaehldatenHeatmapDTO): boolean {
  ...
  private createHeatmapData(zaehldatenHeatmap: LadeZaehldatenHeatmapDTO): void {
    this.resetData();
    ...
    this.seriesEntriesFirstChart = zaehldatenHeatmap.seriesEntriesFirstChart;
    if (zaehldatenHeatmap.seriesEntriesSecondChart !== null) {
      this.seriesEntriesSecondChart = zaehldatenHeatmap.seriesEntriesSecondChart;
    }
  ...
  private formatYAxisLabel(value: any): string {
    return `<span style="color:${value.color}">\u25CF</span> ${value.name}: ${value.data[2].toLocaleString()}<br/>`;
  }

...
export default class BelastungsplanKreuzungSvgSchematischeUebersicht extends Vue {
  ...
  fontfamily: string = "Roboto, Arial, Helvetica, sans-serif"

  // die Basis Werte zum Errechnen der Positionen
  viewbox: number = 1400

  // Prozentwerte um die Strecken zu errechnen
  prozentDiagram: number = 0.9
  prozentSpalt: number = 0.05
  prozentMaxFahrtrichtungWidth: number = 0.12

  // Wert, um wie viel die Knotenarmnummer vergößert werden soll
  knotenarmnummerVergroesserung: number = 110;
  ...
  vonMaxValue: number = 0
  nachMaxValue: number = 0
  lineWidth: number = 0
  fahrtrichtungWidth: number = 0
  highestFahrbeziehungsValue: number = 0
  lowestFahrbeziehungsValue: number = 1000000

  fahrbeziehungsTypen: Map<number, BelastungsplanFahrbeziehung[]> = new Map<number, BelastungsplanFahrbeziehung[]>()
  knotenarme: Map<number, BelastungsplanKnotenarm> = new Map<number, BelastungsplanKnotenarm>()
  ...
  mounted() {
    this.canvas = SVG.SVG().addTo('#belastungsplanSchematischeUebersicht').size(this.dimension, this.dimension).viewbox(0, 0, this.viewbox, this.viewbox);
    this.$store.dispatch("setSizeBelastungsplanSvgSchematischeUebersicht", this.sizeBelastungsplan.replace('px', ''));
    this.$store.dispatch("setMaxSizeBelastungsplanSvgSchematischeUebersicht", this.maxSizeBelastungsplan);
    this.$store.dispatch("setMinSizeBelastungsplanSvgSchematischeUebersicht", this.minSizeBelastungsplan);
  }
    ...
    if (this.fahrbeziehungsTypen.has(knotenarmnummer) && knotenarm) {
      ...
      knotenarmGroup.text(add => {
        add.tspan(knotenarmnummer + "")
          ...
          .font({size: this.lineWidth + this.knotenarmnummerVergroesserung, family: this.fontfamily, anchor: "middle"})
          .attr("alignment-baseline", "central")
          .cx(this.chartPosition + this.ecke - this.spalt * 4) // Radius des Kreises
          .cy(this.viewbox / 2) // Einrückungstiefe der Nummer
          .rotate(r)
      knotenarmGroup.circle(this.lineWidth + this.knotenarmnummerVergroesserung)
  ...
  get sizeBelastungsplan(): string {
    let sizeBelastungsplanSvg: number = this.$store.getters.getSizeBelastungsplanSvgSchematischeUebersicht;

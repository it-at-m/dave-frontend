...
const fahrbeziehungsTypen: Ref<Map<number, BelastungsplanFahrbeziehung[]>> =
    ref(new Map<number, BelastungsplanFahrbeziehung[]>());
const knotenarme: Ref<Map<number, BelastungsplanKnotenarm>> = ref(
    new Map<number, BelastungsplanKnotenarm>()
);
const prozentWerte: Ref<Map<number, boolean>> = ref(new Map<number, boolean>());

const canvas: Ref<SVG.Svg> = ref(SVG.SVG());
const documentationGroup: Ref<SVG.G> = ref(canvas.value.group());
    ...
    if (fahrbeziehungsTypen.value.has(knotenarmnummer) && knotenarm) {
        ...
        fbts.forEach((fbt) => {
            ...
            if (fbt.fahrbeziehungsTyp === Fahrtrichtungsarten.RECHTS90) {
                const zielX =
                    startX + ecke.value + spalt.value + nachPosition * line;

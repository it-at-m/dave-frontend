<template>
  ...
  >
    <v-row no-gutters style="background-color: #EF9A9A"> <!-- red lighten-3 -->
      <v-col cols="12" md="1" v-if="sonderzaehlung" align-self="center" :class="isLarge ? '' : 'px-1'">
        <sonderzaehlung-icon :large="isLarge"/>
      </v-col>
      <v-col cols="12" :md="getMaxCols">
        <v-sheet
            width="100%"
            ...
            @click="openZaehlung"
        >
          <span class="body-2">{{ $d(date, 'short', 'de-DE') }} | {{ projektName }}</span><br/>
          <zaehlart-icon dense :zaehlart="zaehlart" :color="iconColor"></zaehlart-icon>
          <wetter-icon dense :wetter="wetter" :color="iconColor"></wetter-icon>
          <zaehldauer-icon dense :zaehldauer="zaehldauer" :color="iconColor"></zaehldauer-icon>
          <quelle-icon dense :quelle="quelle" :color="iconColor"></quelle-icon>
        </v-sheet>
      </v-col>
    </v-row>
  </v-hover>

  ...
  <v-expansion-panel>
    ...
    <v-expansion-panel-content class="mt-1">
      <panel-header font-size="0.875rem" font-weight="bold" padding="10px 0 0 0"
                    header-text="Verkehrsarten"></panel-header>
      <panel-header font-size="small" font-weight="normal"
                    header-text="Im Belastungsplan können maximal 3 Werte gleichzeitig angezeigt werden"></panel-header>
      <v-row align="start" justify="center" dense>
        <v-col cols="8">
          <v-row align="start" justify="center" dense>
            <v-col cols="6">
              <v-hover v-model="hoverKfz">
                <v-checkbox v-model="fahrzeugOptions.kraftfahrzeugverkehr"
          ...
          <v-row align="start" justify="center" dense>
            <v-col cols="6">
              <v-hover v-model="hoverSv">
                <v-checkbox v-model="fahrzeugOptions.schwerverkehr"
                            :label="'Schwerverkehr (SV)'"
                            :prepend-icon="getIcon('SV')"
                            :hint="getHintToDisplay('SV')"
                            :color="getCheckboxColor('SV')"
                            :persistent-hint="fahrzeugOptions.schwerverkehr || isTypeDisabled('SV')"
            ...
            <v-col cols="6">
              <v-hover v-model="hoverSv_p">
                <v-checkbox v-model="fahrzeugOptions.schwerverkehrsanteilProzent"
                            :label="'Schwerverkehrsanteil [%]'"
                            :prepend-icon="getIcon('SV_P')"
                            :hint="getHintToDisplay('SV_P')"
                            :color="getCheckboxColor('SV_P')"
                            :persistent-hint="fahrzeugOptions.schwerverkehrsanteilProzent || isTypeDisabled('SV_P') || isDifferenzdatenvergleichActive"
                            :disabled="isTypeDisabled('SV_P') || isDifferenzdatenvergleichActive"
          ...
          <v-row align="start" justify="center" dense>
            <v-col cols="6">
              <v-hover v-model="hoverGv">
                <v-checkbox v-model="fahrzeugOptions.gueterverkehr"
                            :label="'Güterverkehr (GV)'"
                            :prepend-icon="getIcon('GV')"
                            :hint="getHintToDisplay('GV')"
                            :color="getCheckboxColor('GV')"
                            :persistent-hint="fahrzeugOptions.gueterverkehr || isTypeDisabled('GV')"
            ...
            <v-col cols="6">
              <v-hover v-model="hoverGv_p">
                <v-checkbox v-model="fahrzeugOptions.gueterverkehrsanteilProzent"
                            :label="'Güterverkehrsanteil [%]'"
                            :prepend-icon="getIcon('GV_P')"
                            :hint="getHintToDisplay('GV_P')"
                            :color="getCheckboxColor('GV_P')"
                            :persistent-hint="fahrzeugOptions.gueterverkehrsanteilProzent || isTypeDisabled('GV_P') || isDifferenzdatenvergleichActive"
                            :disabled="isTypeDisabled('GV_P') || isDifferenzdatenvergleichActive"
                            :hide-details="!isDifferenzdatenvergleichActive"
                            dense></v-checkbox>
              </v-hover>
            </v-col>
          ...
          <v-row align="start" justify="center" dense>
            <v-col cols="6">
              <v-hover v-model="hoverRad">
                <v-checkbox v-model="fahrzeugOptions.radverkehr"
                            :label="'Radverkehr (Rad)'"
                            :prepend-icon="getIcon('RAD')"
                            :hint="getHintToDisplay('RAD')"
                            :color="getCheckboxColor('RAD')"
                            :persistent-hint="fahrzeugOptions.radverkehr || isTypeDisabled('RAD') || isTageswertAndNot24h"
                            :disabled="isTypeDisabled('RAD')"
                            :hide-details="!(isTageswertAndNot24h && fahrzeugOptions.radverkehr)"
                            dense></v-checkbox>
              </v-hover>
            </v-col>

            <v-col cols="6">
              <v-hover v-model="hoverSelectOrDeselectAllVerkehrsarten">
                <v-checkbox v-model="selectOrDeselectAllVerkehrsartenVmodel"
                            value=""
          ...
          <v-row align="start" justify="center" dense>
            <v-col cols="6">
              <v-hover v-model="hoverFuss">
                <v-checkbox v-model="fahrzeugOptions.fussverkehr"
                            :label="'Fußgänger (Fuß)'"
      ...
      <v-divider></v-divider>
      <panel-header font-size="0.875rem" font-weight="bold" padding="10px 0 0 0"
                    header-text="Fahrzeugkategorien"></panel-header>
      <panel-header font-size="small" font-weight="normal"
                    header-text="(keine Anzeige im Belastungsplan)"></panel-header>

      <v-row align="start" justify="center" dense>
        <v-col cols="4">
          <v-hover v-model="hoverPkw">
            <v-checkbox v-model="fahrzeugOptions.personenkraftwagen"
          ...
          <v-hover v-model="hoverLkw">
            <v-checkbox v-model="fahrzeugOptions.lastkraftwagen"
          ...
          <v-hover v-model="hoverLz">
            <v-checkbox v-model="fahrzeugOptions.lastzuege"
                        :label="'Lastzüge (Lz)'"
          ...
          <v-hover v-model="hoverBus">
            <v-checkbox v-model="fahrzeugOptions.busse"
                        :label="'Bus'"
        ...
        <v-col cols="4">
          <v-hover v-model="hoverKrad">
            <v-checkbox v-model="fahrzeugOptions.kraftraeder"
          ...
          <v-hover v-model="hoverPkweinheit">
            <v-checkbox v-model="fahrzeugOptions.pkwEinheiten"
                        :label="'PKW-Einheiten'"
...
export default class FahrzeugPanel extends Vue {
  ...
  private svAnteilForDifferenzdatenSaved: boolean = true;
  private gvAnteilForDifferenzdatenSaved: boolean = true;
  ...
  private selectOrDeselectAllVerkehrsartenVmodel: boolean = false;
  private hoverSelectOrDeselectAll: boolean = false;
  private hoverSelectOrDeselectAllVerkehrsarten: boolean = false;
  ...
  get labelSelectOrDeselectAllVerkehrsarten(): string {
    return this.selectOrDeselectAllVerkehrsartenVmodel ? 'Alles abwählen' : 'Alles auswählen';
  }
  ...
  private differenzdatenvergleichChanged() {
    if (this.isDifferenzdatenvergleichActive) {
      ...
      this.svAnteilForDifferenzdatenSaved = this.fahrzeugOptions.schwerverkehrsanteilProzent;
      this.gvAnteilForDifferenzdatenSaved = this.fahrzeugOptions.gueterverkehrsanteilProzent;
      ...
      this.fahrzeugOptions.schwerverkehrsanteilProzent = this.svAnteilForDifferenzdatenSaved;
      this.fahrzeugOptions.gueterverkehrsanteilProzent = this.gvAnteilForDifferenzdatenSaved;
    }
  }
  ...
  private selectOrDeselectAll() {
    if (!this.isTypeDisabled(Fahrzeug.PKW)) {
      this.fahrzeugOptions.personenkraftwagen = this.selectOrDeselectAllVmodel;
    }
    if (!this.isTypeDisabled(Fahrzeug.LKW)) {
      this.fahrzeugOptions.lastkraftwagen = this.selectOrDeselectAllVmodel;
    }
  ...
  private selectOrDeselectAllVerkehrsarten() {
    if (!this.isTypeDisabled(Fahrzeug.KFZ)) {
      this.fahrzeugOptions.kraftfahrzeugverkehr = this.selectOrDeselectAllVerkehrsartenVmodel;
    }
    if (!this.isTypeDisabled(Fahrzeug.SV)) {
      this.fahrzeugOptions.schwerverkehr = this.selectOrDeselectAllVerkehrsartenVmodel;
    }
    if (!this.isTypeDisabled(Fahrzeug.GV)) {
      this.fahrzeugOptions.gueterverkehr = this.selectOrDeselectAllVerkehrsartenVmodel;
    }
    if (!this.isTypeDisabled(Fahrzeug.SV_P)) {
      this.fahrzeugOptions.schwerverkehrsanteilProzent = this.selectOrDeselectAllVerkehrsartenVmodel;
    }
    if (!this.isTypeDisabled(Fahrzeug.GV_P)) {
      this.fahrzeugOptions.gueterverkehrsanteilProzent = this.selectOrDeselectAllVerkehrsartenVmodel;
    }
    if (!this.isTypeDisabled(Fahrzeug.RAD)) {
      this.fahrzeugOptions.radverkehr = this.selectOrDeselectAllVerkehrsartenVmodel;
    }
    if (!this.isTypeDisabled(Fahrzeug.FUSS)) {
      this.fahrzeugOptions.fussverkehr = this.selectOrDeselectAllVerkehrsartenVmodel;
    }
  }
  ...
  get isTageswertAndNot24h(): boolean {
    return (this.fahrzeugOptions.zeitauswahl === Zeitauswahl.TAGESWERT)
        && this.activeZaehlung.zaehldauer !== Zaehldauer.DAUER_24_STUNDEN;
  }

  get isAnyKFZselected(): boolean {
    return this.fahrzeugOptions.kraftfahrzeugverkehr || this.fahrzeugOptions.schwerverkehr || this.fahrzeugOptions.gueterverkehr
      || this.fahrzeugOptions.schwerverkehrsanteilProzent || this.fahrzeugOptions.gueterverkehrsanteilProzent;
  }
    ...
    switch (type) {
      case "RAD": {
        ...
        if (this.isTageswertAndNot24h) {
          hint = 'Hinweis: Die Tageswerte für den Radverkehr wurden vom KI-Modul hochgerechnet.';
          if (this.isAnyKFZselected) {
            hint = 'Der Tageswert für den Radverkehr kann nicht gleichzeitg mit KFZ-Werten im Belastungsplan zusammen angezeigt werden.';
          }
        }
        break;
      }
      case "FUSS": {
        if (this.isTageswertAndNot24h) {
          hint = 'Achtung: Der Fußverkehr verfügt über keinen Tageswert. Zur Anzeige im Belastungsplan die Zeitauswahl ändern.';
        }
        break;
      }
      case "SV_P": {
  ...
  private getCheckboxColor(type: string): string {
    ...
    switch (type) {
      ...
      case "FUSS": {
        if (this.fahrzeugOptions.fussverkehr && this.actualNumberOfSelectedVerkehrsarten > 1) {
          color = "grey darken-1";
        }
        if (this.isTageswertAndNot24h) {
          color = "grey darken-1";
  ...
  private getIcon(type: string): string {
    ...
    switch (type) {
      ...
      case "FUSS": {
        // Angezeigt, wenn FUSS
        if (!this.isTageswertAndNot24h && this.fahrzeugOptions.fussverkehr && this.actualNumberOfSelectedVerkehrsarten === 1) {
          icon = `mdi-arrow-decision`;
        }
        break;
      }
  ...
  private calculateSelectOrDeselectVerkehrsarten() {
    ...
    // counter > maxSelectable/2 => abwählen
    // sonst => auswählen
    this.selectOrDeselectAllVerkehrsartenVmodel = counter > (maxSelectable / 2);
  }

  // Verkehrsarten
  private hoverKfz: boolean = false;
  private hoverSv: boolean = false;
  private hoverGv: boolean = false;
  private hoverSv_p: boolean = false;
  private hoverGv_p: boolean = false;
  private hoverRad: boolean = false;
  private hoverFuss: boolean = false;

  // Fahrzeugkategorien
  private hoverPkw: boolean = false;
  private hoverLkw: boolean = false;
  private hoverLz: boolean = false;
  private hoverBus: boolean = false;
  private hoverKrad: boolean = false;
  private hoverPkweinheit: boolean = false;
  ...
  get helpTextVerkehrsarten(): string {
    ...
    if (this.hoverSelectOrDeselectAllVerkehrsarten) {
      let text: string = "Aktiviert alle Verkehrsarten.";
  ...
  get helpTextFahrzeugkategorien(): string {
    ...
    if (this.hoverSelectOrDeselectAll) {
      let text: string = "Aktiviert alle Fahrzeugkategorien.";
  ...
  get isSv_pInBelastungsPlan(): boolean {
    let actualNumberOfSelectedKfzSvAndGv: number = 0;
    this.fahrzeugOptions.kraftfahrzeugverkehr ? actualNumberOfSelectedKfzSvAndGv++ : '';
    this.fahrzeugOptions.schwerverkehr ? actualNumberOfSelectedKfzSvAndGv++ : '';
    this.fahrzeugOptions.gueterverkehr ? actualNumberOfSelectedKfzSvAndGv++ : '';
    return this.fahrzeugOptions.schwerverkehrsanteilProzent && (this.fahrzeugOptions.kraftfahrzeugverkehr || this.fahrzeugOptions.schwerverkehr || this.fahrzeugOptions.gueterverkehr) && actualNumberOfSelectedKfzSvAndGv < 3;
  }
  ...
  get isGv_pInBelastungsPlan(): boolean {
    let actualNumberOfSelectedKfzSvGvAndSV_P: number = 0;
    this.fahrzeugOptions.kraftfahrzeugverkehr ? actualNumberOfSelectedKfzSvGvAndSV_P++ : '';
    this.fahrzeugOptions.schwerverkehr ? actualNumberOfSelectedKfzSvGvAndSV_P++ : '';
    this.fahrzeugOptions.gueterverkehr ? actualNumberOfSelectedKfzSvGvAndSV_P++ : '';
    this.fahrzeugOptions.schwerverkehrsanteilProzent ? actualNumberOfSelectedKfzSvGvAndSV_P++ : '';
    return this.fahrzeugOptions.gueterverkehrsanteilProzent && (this.fahrzeugOptions.kraftfahrzeugverkehr || this.fahrzeugOptions.schwerverkehr || this.fahrzeugOptions.gueterverkehr) && actualNumberOfSelectedKfzSvGvAndSV_P < 3;
  }

  get isRadInBelastungsplan() : boolean {
    return this.fahrzeugOptions.radverkehr && this.actualNumberOfSelectedVerkehrsarten === 1;
  }
  ...
  adaptFahrzeugauswahl(options: OptionsDTO): OptionsDTO {
    if (options.zeitauswahl === Zeitauswahl.SPITZENSTUNDE_RAD || options.zeitauswahl === Zeitauswahl.SPITZENSTUNDE_FUSS) {

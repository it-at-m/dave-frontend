      ...
      <v-hover v-model="hoverSelectBasisdatum">
        <div>
          <panel-header font-size="0.875rem" font-weight="bold" padding="10px 0 0 0"
                        header-text="Datum der aktuell ausgewählten Zählung"></panel-header>
          <span class="text">{{ basisdatum }}</span><br/><br/>
        </div>
      </v-hover>
      <v-row align="start" justify="center" dense>
        <v-col cols="4">
          <panel-header font-size="0.875rem" font-weight="bold" padding="10px 0 0 0"
                        header-text="Differenzdaten anzeigen"></panel-header>
          <panel-header font-size="small" font-weight="normal"
                        header-text="(Anzeige nur im Belastungsplan)"></panel-header>

          <v-hover v-model="hoverCheckbox">
            <v-checkbox v-model="differenzdatenDarstellen"
                        color="grey darken-1"
                        :label="'Differenzdaten darstellen'"></v-checkbox>
          </v-hover>
...
export default class ZaehlungsvergleichPanel extends Vue {
  ...
  private vergleichszaehlungsId: string = "";
  private differenzdatenDarstellen: boolean = false;
  private vergleichsdatumDifferenzdarstellung: Array<KeyVal> = new Array<KeyVal>();

  // Zeitreihe
  private vergleichsdatumZeitreihe: Array<KeyVal> = new Array<KeyVal>();
  private idVergleichszaehlungZeitreihe: string = '';

  private currentZeitblock: string = "";
  ...
  updateIdVergleichszaehlungZeitreihe() {
    this.$emit("idVergleichszaehlungZeitreihe", this.idVergleichszaehlungZeitreihe)
  }
  ...
  zeitblockChanged(newZeitblock: string) {
    ...
    this.vergleichsdatumCalculator(this.currentZeitblock, this.currentZeitauswahl);
    this.zeitreihenVergleichsdatumCalculator(this.currentZeitblock, this.currentZeitauswahl);
  }
  ...
  get basisdatum(): string {
    ...
    if (this.zaehlung?.datum) {
      result = `${this.$d(this.datum(this.zaehlung.datum), 'short', 'de-DE')}`;
    }
    return result;
  }
  ...
  private hoverSelectBasisdatum: boolean = false;
  private hoverSelectVergleichsdatum: boolean = false;
  private hoverCheckbox: boolean = false;
  private hoverSelectVergleichsdatumZeitreihe: boolean = false;
  ...
  get helpTextDifferenzdatenBelastungsplan(): string {
    ...
    if (this.hoverSelectVergleichsdatumZeitreihe) {
      return "Datum der Zählung, bis zu der die Zeitreihe angezeigt werden soll (inklusive).\n" +
  ...
  update(newOptions: OptionsDTO) {
    this.vergleichsdatumCalculator(newOptions.zeitblock, newOptions.zeitauswahl);
    this.zeitreihenVergleichsdatumCalculator(newOptions.zeitblock, newOptions.zeitauswahl);
    ...
    newOptions.differenzdatenDarstellen === null
        ...
        : this.differenzdatenDarstellen = newOptions.differenzdatenDarstellen;
    newOptions.idVergleichszaehlungZeitreihe === null
        ? this.idVergleichszaehlungZeitreihe = ""
        : this.idVergleichszaehlungZeitreihe = newOptions.idVergleichszaehlungZeitreihe;
  }
   ...
   */
  private vergleichsdatumCalculator(newZeitblock: string, newZeitauswahl: string): void {
    const result: Array<KeyVal> = new Array<KeyVal>();
    const zaehlungen: Array<LadeZaehlungDTO> = this.$store.getters.getZaehlungen;
    if (this.zaehlung?.id) {
      zaehlungen.forEach(zaehl => {
        if (zaehl.id !== this.zaehlung?.id
            && zaehl.zaehlart === this.zaehlung?.zaehlart
            && zaehl.sonderzaehlung === this.zaehlung?.sonderzaehlung
            && (this.containsZeitblock(zaehl, newZeitblock) || newZeitauswahl.toString() === Zeitauswahl.TAGESWERT.toString())) {
          result.push({text: `${this.$d(this.datum(zaehl.datum), 'short', 'de-DE')}`, value: zaehl.id});
        }
      });
    }
   ...
   * - Welche den gewählten Zeitblock besitzt.
   * */
  private zeitreihenVergleichsdatumCalculator(newZeitblock: string, newZeitauswahl: string): void {
    const result: Array<KeyVal> = new Array<KeyVal>();
    const zaehlungen: Array<LadeZaehlungDTO> = this.$store.getters.getZaehlungen;
    if (this.zaehlung?.id != undefined && this.zaehlung?.datum != undefined) {
      zaehlungen.forEach(zaehl => {
        if (zaehl.id !== this.zaehlung?.id
   ...
   */
  private containsZeitblock(zaehlung: LadeZaehlungDTO, zeitblockInOptions: string): boolean {
    return zaehlung.zeitauswahl.blocks.some(zeitblock => zeitblock.toString() === zeitblockInOptions)
        || zaehlung.zeitauswahl.hours.some(zeitblockStuendl => zeitblockStuendl.toString() === zeitblockInOptions);
  }
